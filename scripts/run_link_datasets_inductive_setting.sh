#!/usr/bin/env bash
# DyGLib inductive (new-node) setting run (random negatives), kept apart from
# the random-negative run: it reads configs/link_comparison_all_inductive_setting.yaml
# (an overlay of link_comparison_all.yaml) and writes results/inductive_setting/*.
# Usage mirrors run_link_datasets.sh:
#   bash scripts/run_link_datasets_inductive_setting.sh [datasets...]
#   MODELS="rcps_jepa dygformer" SEEDS="0 1" bash scripts/run_link_datasets_inductive_setting.sh canparl
...
cd "$PROJECT_DIR"
mkdir -p logs results/inductive_setting
...
COMMAND=(
  ...
  --config configs/link_comparison_all_inductive_setting.yaml
)
if [[ "$#" -gt 0 ]]; then
  COMMAND+=(--datasets "$@")
fi
