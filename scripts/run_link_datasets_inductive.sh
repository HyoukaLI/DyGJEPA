#!/usr/bin/env bash
# Inductive negative sampling run (DyGLib/EdgeBank protocol), kept apart from
# the random-negative run: it reads configs/link_comparison_all_inductive.yaml
# (an overlay of link_comparison_all.yaml) and writes results/inductive/*.
# Usage mirrors run_link_datasets.sh:
#   bash scripts/run_link_datasets_inductive.sh [datasets...]
#   MODELS="rcps_jepa dygformer" SEEDS="0 1" bash scripts/run_link_datasets_inductive.sh canparl
...
cd "$PROJECT_DIR"
mkdir -p logs results/inductive
...
COMMAND=(
  ...
  --config configs/link_comparison_all_inductive.yaml
)
if [[ "$#" -gt 0 ]]; then
  COMMAND+=(--datasets "$@")
fi
